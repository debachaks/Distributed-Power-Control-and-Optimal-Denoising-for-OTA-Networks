...
def adaptive_eta_best_response(
    ...
    power_outer_history: list[npt.NDArray[np.float64]] = []
    utility_outer_history: list[npt.NDArray[np.float64]] = []
    ...
    while (convergence_eta == 0) and (iteration_eta < max_iter):
        ...
        if np.abs(mse_history[-1] - mse_history[-2]) < epsilon_mse:
            convergence_eta = 1
